...
## D65. FlowCanvas: прозрачность нод + убрана клип-рамка канвы (2026-07-01)

- **Прозрачность (как на референсе).** Ноды на канве — `opacity: 0.9`: точечный грид просвечивает сквозь карточки (раньше opacity 1 — грид не был виден за нодами). Порты/меню (сиблинги FlowNode) остаются полностью непрозрачными.
- **«Толстая рамка» у краёв убрана.** У `.ds-flow-canvas` сняты `border-radius` (скруглённая рамка вокруг грид-области) и `overflow: hidden` (клипал outline выделенной ноды у края в «рамку»). Теперь `overflow: visible`; безопасно: ноды клампятся в границы сами, wires клипаются самим `<canvas>` (raster-bounds), грид ограничен размером div. Page overflow остаётся 0.
- Полосы на фоне из скрина — артефакт сжатия скриншота (фон страницы чистый: `bg-base`, без bg-image; панели непрозрачные) — реального CSS-источника нет.
- Проверка (DOM): overflow=0, node opacity 0.9, canvas border-radius 0 / overflow visible, грид+wires рисуются.

## D66. Automation: свободный drag+наложение нод + фон-колонки (Figma layout grid) (2026-07-01)

- **«Невидимая толстая рамка» = жёсткий кламп** позиции ноды в границы канвы ([0, W−NODE_W]) — у краёв нода упиралась в стену и не могла полностью наложиться на соседа рядом с краем. Заменил на мягкий кламп: `x ∈ [MIN_VISIBLE−NODE_W, W−MIN_VISIBLE]` (MIN_VISIBLE=40) — ноды свободно накладываются друг на друга и уводятся за край, но ≥40px всегда видно (не теряются). `overflow:hidden` на канве контейнит вынос за край (без паразитного скролла), `border-radius` нет, selected-outline `outline-offset:-3` (внутрь — не клипается в «рамку»). Проверка (DOM): Final уводится на x=570 (за старый лимит 340); Screening полностью накладывается на Interview (40,360); overflow=0.
- **Фон-колонки** (Figma layout grid узла 1:4230: COLUMNS, count 8, offset 20 (=indent-m), gutterSize 8 (=indent-xs), STRETCH, sage `#a1bfb5` @10%). Реализовано на странице Automation: слой `.ds-column-grid` (absolute inset-0, z-0, pointer-events:none) с 8 flex-колонками `flex-1`, gap indent-xs, padding-inline indent-m, фон `var(--ds-color-grid-column)`. Контент обёрнут в `relative z-10`. Цвет — новый токен `--ds-color-grid-column: rgba(161,191,181,0.1)` (guide-цвет из макета, не семантический DS-цвет). Дот-грид канвы убран (в Figma фон страницы = колонки; канва прозрачная — колонки просвечивают сквозь неё и полупрозрачные ноды). Проверка (DOM): 8 колонок, width 168 (@1440), gutter 8, offset 20/20, цвет rgba(161,191,181,0.1), высота = вся страница, z-0, pointer-events none, клики проходят к нодам/панелям, overflow=0.

## D67. Automation: ноды не режутся у краёв + зазор панель↔канва 8px (Figma) (2026-07-01)

- **«Блок зарезается».** D66-мягкий кламп (`x ∈ [40−W, W−40]`) уводил ноду за границу канвы, а `overflow:hidden` её там обрезал (виден на узких вьюпортах, где `fit()` прижимал Final к правому краю). Вернул **строгий кламп в границы** (`x ∈ [0, W−NODE_W]`, `y ∈ [0, H−NODE_H]`) при drag — нода всегда целиком в поле канвы, не режется; наложение друг на друга сохраняется (обе ноды в поле). `overflow` канвы → `visible` (страховка: даже нода вплотную к краю не клипается; за канву не выходит благодаря клампу → паразитного скролла нет). Убрал неиспользуемую константу MIN_VISIBLE.
- **«Серая рамка рядом с белым блоком» = слишком большой зазор.** Между канвой и панелями стоял `gap-ds-xl` (30px) — гулкая серая полоса у белой панели. В Figma (Frame 1407) зазор панель↔канва = **8px** (`indent-xs`), наружные поля = 30px. Поставил `lg:gap-ds-xs` (8px на десктопе; на мобиле-стопке остаётся `gap-ds-xl`). Побочный бонус: канва стала 664px — точь-в-точь как в макете.
- Проверка (DOM @1200 и @1440): clippedNodes=0 (лево/право), pageOverflow=0, gapLeft=gapRight=8, canvasW=664 (@1440). Скрин флапает по загрузке шрифтов — замеры DOM исчерпывающи.

## D68. Automation: задвиг нод за боковые панели + сворачиваемая панель свойств (2026-07-01)

- **Задвиг нод за панели.** Пользователь: «блок упирается в стену — хочу задвигать за белые панели слева/справа». Горизонтальный кламп ослаблен: `x ∈ [MIN_VISIBLE−NODE_W, W−MIN_VISIBLE]` (MIN_VISIBLE=48) — ноду можно увести за край, ≥48px остаётся видно в окне канвы (не потеряется, вытащить обратно). За краем `overflow:visible` уводит ноду ПОД боковые панели (панели `relative z-10`, канва `main relative z-0` — в стек-контексте content-wrapper), а не режет. Вертикаль — строго в поле (`y ∈ [0, H−NODE_H]`). Горизонтальный вынос клипается на уровне ряда `.ds-automation-row` (`overflow-x:clip` — не создаёт скролл-контейнер, не трогает вертикаль/ScrollRestoration) → скролла страницы нет. Проверка (реальный drag @1440): нода задвинута на 232px за правый край, `elementFromPoint` в зоне перекрытия = панель (нода под ней), pageOverflow=0.
- **Сворачиваемая панель свойств.** Node Properties сворачивается кнопкой-шевроном в узкий rail (`lg:w-11`=44px) с кнопкой развернуть (вертикальный лейбл на rail). Освободившееся место забирает канва (`flex-1`): проверено 664→970px. Состояние формы (nodeName/props) сохраняется (стейт в Automation, не размонтируется). Иконки — inline SVG-шевроны (currentColor), фокус — `var(--ds-focus-ring)`, hover — `bg-control-secondary` (токены, без хардкода).

## D69. CLAUDE_default.md — дефолтный контекст проекта (local-only) (2026-07-01)

- **Запрос (FULL GAS).** Собрать свод правил/выводов/опыта проекта в один файл `CLAUDE_default.md`,
  чтобы новый агент, прочитав только его, работал в стиле проекта без доп. объяснений.
- **Содержание** (durable-правила, без разовых деталей): стек/раскладка; процесс FULL GAS
  (решения→DECISIONS, журнал→NOTES, release-notes на каждое изменение, гейт check-docs, security
  перед push/deploy); Figma↔React (источник правды, не изобретать, не дублировать, зеркальный нейминг,
  без хардкода, направление токен→компонент→страница, Figma побеждает конфликт восприятия); шпаргалка
  токенов + типографический паттерн (Pixform-контролы 10px UPPER / Serif-дисплей / Akkurat-body);
  pixel-parity (метод сверки, статусы ✅/⚠️/❌, 0❌); **топ-10 типовых ошибок проекта с чеклистами**
  (обрезанные бары, лишний blur/overlay, съехавший центр hero, правый overflow/100vw, скролл
  контейнером, бледные аватары, выдуманные компоненты, hero не full-size, потерянные разделители,
  размеры dropdown); атомарные этапы Prepare→Styles→Atoms→Molecules→Organisms→Screens; финальный чеклист.
- **Источники извлечения:** DECISIONS.md (D1–D68), CONTRIBUTING.md, audit/parity/INDEX.md, theme.css, код.
- **Решение по хранению (указание пользователя):** файл держим **локально** — добавлен в `.gitignore`,
  в репозиторий не коммитится и в пуш не попадает. Причина: это системный контекст агента, не артефакт
  продукта. Запись в release-notes/NOTES оставлена (фиксирует факт наличия локального свода).

## D70. Табы шапки — реальная навигация по страницам + логотип→каталог (2026-07-01)

- **Запрос.** «Сделай кнопки на табах активными и чтобы кнопки переходили по страницам.» Табы шапки
  были локальным стейтом (`useState('teams')`) — переключали подсветку, но никуда не вели.
- **Решение.** Табы = навигация по 5 реальным экранам. Единый источник `usePageNav`
  (`src/pages/pageNav.tsx`): список `PAGE_NAV` (All teams/Team/Candidate/Campaign/Automation → роуты
  `/pages/*`); `value` активного таба вычисляется по `useLocation().pathname` (подсветка = текущая
  страница), `onTabChange` делает `navigate(path)`. Подключено в `PageFrame` (4 страницы) и `Automation`
  (свой инлайн-хедер) — без дублей и хардкода переходов. Логотип «Hired & Wired» — кликабельная кнопка
  (`toHome` → `/`, каталог); стили наследуются (`[font:inherit]`, прозрачный фон), hover-opacity.
- **Отступление от Figma (осознанное, по прямому запросу).** В макете шапки 2 таба (All teams/All
  templates) — заменены на навигацию из 5 экранов, т.к. пользователь явно просил переходы между
  страницами (прямая инструкция > статичный макет; правило CLAUDE_default §2). Компонент/стиль таба
  (MenuSwitch) не менялся — только назначение и количество.
- **Фикс overflow.** 5 табов + CTA не влезали в один ряд → +157px горизонтального overflow на 375.
  `.ds-topmenu__controls` и `.ds-topmenu__tabs` получили `flex-wrap` (переносятся на узких). Десктоп-вид
  не изменился.
- **Проверка (реальные клики + DOM):** на Automation активен таб «Automation»; клик «All teams» →
  `/pages/all-teams` (активным стал «All teams»); клик «Candidate» → `/pages/candidate`; логотип → `/`;
  0 console errors; overflow 0 на 1440/768/375 по всем страницам.

## D71. CardTop: теги-дропдауны раскрываются ВНИЗ (2026-07-01)

- **Запрос.** «Сделай, чтобы открывались вниз» (скрин Candidate: тег-дропдаун раскрылся ВВЕРХ).
- **Причина upward в D57.** У `.ds-card-top` стоял `overflow:hidden` → меню, открытое вниз, обрезалось
  (теги внизу карточки). Поэтому D57 форсил меню вверх (`top:auto; bottom:100%`).
- **Решение.** Снял `overflow:hidden` с `.ds-card-top` (→ `overflow:visible`) — фон-картинку клипает её
  СОБСТВЕННЫЙ слой `.ds-card-top__bg` (own `overflow:hidden` + `border-radius:inherit`), а фон-цвет
  карточки клипается `border-radius`, поэтому скруглённый вид сохраняется. Оверрайд меню вернул к базовому
  ВНИЗ (`top:calc(100%+xxs); bottom:auto`, `min-width:180`). Меню теперь выходит за низ карточки и видно
  поверх контента (z-index 20 базового Dropdown; блок тегов — `position:relative;z-index:1` создаёт
  стек-контекст → меню рисуется над статичным `main` ниже). Отменяет «меню вверх» из D57.
- **Проверка (реальный клик @1440):** Candidate — меню НИЖЕ контрола (menuTop==controlBottom), выходит за
  низ карточки, `elementFromPoint` = меню (не обрезано/не перекрыто); фон клипается своим слоем
  (`.ds-card-top__bg` overflow hidden), imgWithinCard=true; page overflow 0 на всех страницах (1440/375),
  0 console errors. Glass-hero (AllTeams/HiringCampaign) без регрессий.

## D72. Убраны чёрные UA-рамки нативных контролов (preflight off) (2026-07-01)

- **Симптом.** У карточек Node library (и Templates, шеврона панели) на Automation — чёрные рамки,
  которых нет в Figma. Аудит DOM: `border: 2px outset rgb(0,0,0)` — это UA-дефолт нативной `<button>`.
- **Причина.** Preflight Tailwind выключен (D29, чтобы не трогать компоненты) → его сброс
  `*{border-width:0;border-style:solid}` не применяется, и нативные `button/input/select/textarea`
  сохраняют браузерный `outset`-бордер. DS-компоненты (`.ds-*`) свой border задают явно (не страдали),
  а вот raw-Tailwind кнопки на страницах/в каталоге — да.
- **Решение (системное, не по одной кнопке).** В `global.css` — точечный сброс формо-контролов:
  `button, input, optgroup, select, textarea { border-width:0; border-style:solid; border-color:currentColor }`.
  Это ровно то, что делал бы preflight для бордера: без `.border` рамки нет, с утилитой `.border` она
  снова работает (solid). Специфичность элемента (0,0,1) ниже классов → DS-`.ds-*` и утилиты `border-*`
  перебивают, их бордеры сохранены (проверено: `.ds-button--secondary` остался 1px solid green).
- **Проверка (DOM, все роуты).** `outset/inset/groove/ridge`-бордеров: **0** на `/`, `/release-notes`,
  all-teams, candidate, hiring-campaign, automation. Node library card border `0px` (рамка ушла),
  `.ds-button--secondary` green border на месте, 0 console errors. Правило зафиксировано в CLAUDE_default
  (топ типовых ошибок: «нативные рамки при выключенном preflight»).
