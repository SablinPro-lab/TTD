...
## [Страница Automation (1:4230) — node-graph: drag + нитки] — 2026-07-01
Новый организм FlowCanvas: ноды (FlowNode) перетаскиваются (pointer + transform), порты соединяются нитками (SVG bezier, drag выход→вход). Оптимизация: window pointer-события + rAF. FlowNode +color, Header +secondRowActions. Страница Automation: 3 панели (library/canvas/properties) на атомах + FlowCanvas, роут /pages/automation, в навигации Pages. DOM-сверка: цвета узлов #f5cfca/#ddd6ef/#fad5e7, библиотека #f7e0dd×3/#e0e2a4×2/#fad5e7/#ddd6ef, нитки #000 1.5px, drag сдвигает узел+нитки следуют, connect 2→3, 0 ошибок, overflow 0. D58.

## [Automation → настоящий canvas (HTML5) + аудит границ] — 2026-07-01
Связи перенесены с SVG на реальный HTML5 <canvas>: программно по данным, DPR-aware (canvas.width=css*dpr, cap 2.5), ResizeObserver-ресайз, rAF-анимация «сигнала» бегущего по проводу с prefers-reduced-motion (в reduced — статично, перерисовка по изменениям). Цвета из токенов (black/white), grid — CSS. Canvas-слой pointer-events:none, z10 (линии сверху, интерактив под ним кликается — elementFromPoint на порту = dot). Ноды клампятся в границы канвы (0..W-280/0..H-125) при add/drag/mount/resize — нет overflow/обрезаний. Отключение провода — hit-test по семплам безье. Границы: адаптив 3 панелей (стопка <1024, три колонки ≥1024) → overflow=0 на 375/768/1440. DOM: canvas ink>0, drag двигает ноду+перерисовка, disconnect работает, порты чёрный/серый. D63.

## [Аудит границ/отступов всех страниц] — 2026-07-01
Прошёлся по всем 7 страницам (1440+375). @1440 всё чисто: main 830 центр, Team-статусы @625, Pipeline 1220 центр, панели Automation 350/350, canvas не перекрывает интерактив, overflow=0. @375 продуктовые страницы + release-notes + automation overflow=0. Единственная проблема — каталог Preview: широкие десктоп-демо распирали страницу (~173px). Фикс: .ds-demo → min-w-0/max-w-full/overflow-x-auto (демо скроллится внутри карточки). D64.

## [FlowCanvas: прозрачность нод + без клип-рамки] — 2026-07-01
Ноды на канве opacity 0.9 (грид просвечивает, как на референсе). Канва без border-radius/overflow:hidden — нет толстой рамки/клипа у краёв (ноды клампятся сами, wires клипает <canvas>). Фоновые полосы из скрина — артефакт сжатия (фон чистый). DOM: overflow=0, opacity 0.9, grid+wires ок. D65.

## [Automation: свободный drag+наложение + фон-колонки] — 2026-07-01
Drag: убран жёсткий кламп-«стена» → ноды свободно накладываются и уводятся за край (≥40px видно, overflow:hidden контейнит; selected-outline внутрь). Проверено: Final→x570 (за старый лимит 340), Screening полностью на Interview, overflow=0. Фон-колонки как в Figma (layout grid 1:4230: 8 cols, offset 20, gutter 8, sage #a1bfb5 @10
## [Automation: свободный drag+наложение + фон-колонки] — 2026-07-01
Drag: убран жёсткий кламп-«стена» → ноды свободно накладываются и уводятся за край (≥40px видно, overflow:hidden контейнит; selected-outline внутрь). Проверено: Final→x570 (за старый лимит 340), Screening полностью на Interview, overflow=0. Фон-колонки как в Figma (layout grid 1:4230: 8 cols, offset 20, gutter 8, sage #a1bfb5 @10%, токен --ds-color-grid-column) — слой z-0 pointer-events:none под нодами/панелями; канва прозрачная (дот-грид убран). D66.

## 2026-07-01 — D67 Automation: ноды не режутся + зазор 8px
- FlowCanvas: строгий кламп нод в границы канвы (не режутся у краёв), overflow:visible, убран MIN_VISIBLE.
- Automation: зазор панель↔канва 30→8px (Figma Frame 1407) → ушла «серая рамка» у панели; канва = 664px (Figma-exact).
- Проверено DOM @1200/@1440: клипов 0, overflow 0, gap 8/8.

## 2026-07-01 — D68 задвиг нод за панели + сворачивание панели
- FlowCanvas: горизонтальный кламп разрешает задвиг за боковые панели (MIN_VISIBLE=48), overflow:visible → нода уходит ПОД панель (панели z-10 > канва z-0), ряд overflow-x:clip → без скролла страницы.
- Automation: панель Node Properties сворачивается в rail 44px (кнопка-шеврон), канва забирает место (664→970).
- Проверено реальным drag @1440: задвиг 232px за край, нода под панелью, overflow 0; сворачивание — канва растёт, overflow 0.

## 2026-07-01 — CLAUDE_default.md (дефолтный контекст, local-only)
- Создан CLAUDE_default.md в корне — свод durable-правил проекта (процесс FULL GAS, Figma↔React,
  токены/типографика, pixel-parity, топ-10 типовых ошибок + чеклисты, этапы, финальный чеклист).
  Источники: DECISIONS.md (D1–D68), CONTRIBUTING.md, audit/parity/INDEX.md, theme.css, код.
- По решению пользователя файл в .gitignore — используется как системный контекст будущих сессий,
  в репозиторий НЕ коммитится (пуш идёт без него).

## 2026-07-01 — D70 навигация по табам шапки
- Табы шапки теперь ведут по страницам (usePageNav: 5 экранов, active по роуту), логотип → каталог.
- Подключено в PageFrame (4 страницы) + Automation. TopMenu controls/tabs → flex-wrap (5 табов на узких).
- Проверено кликами: переходы работают, активный таб = текущий роут; overflow 0 на 1440/768/375; 0 ошибок.

## 2026-07-01 — D71 CardTop дропдауны вниз
- Теги-дропдауны в hero открываются ВНИЗ (снят overflow:hidden с .ds-card-top; фон клипается своим слоем).
- Отменяет upward из D57. Проверено на Candidate: меню под контролом, не обрезано, overflow 0, без регрессий glass-hero.

## 2026-07-01 — D72 чёрные рамки нативных контролов убраны
- Причина: preflight off (D29) → нативные button/input/select/textarea рисовали UA outset-border.
- Фикс: глобальный сброс border формо-контролов в global.css (width 0 + solid); DS/утилиты border-* не затронуты.
- Аудит всех роутов: 0 outset-рамок; library card без рамки; .ds-button--secondary green border сохранён.
