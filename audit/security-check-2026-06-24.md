...
Рецепт: внутренний security-чек-лист (см. CONTRIBUTING.md → «Security-чек перед push и deploy»). Проверки реальные (git grep / npm audit / чтение файлов).
...
- `/security-review` (официальная команда) **в этой сессии не запускалась**: cwd сессии — корень
  соседнего проекта (не git этого проекта), а git-репо находится в `FIgma tests/`. Для прогона нужно
